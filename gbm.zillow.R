...
gbmGrid <-  expand.grid(interaction.depth = c(7,11,15), 
                      n.trees = (1:10)*50, 
                        shrinkage = c(.001, .01),
                        n.minobsinnode = 10)

gbmFit3 <- train(logerror ~ . - propertycountylandusecode,
                 data = full.train1, 
                 method = "gbm", 
...
gbmImp3 <- varImp(gbmFit2, scale = FALSE)
plot(gbmImp2, top = 10)
...
oct = predict(gbm.caret, newdata=full, type="raw")
full$transactiondate=as.numeric(11)
nov = predict(gbm.caret, newdata=full, type="raw")
full$transactiondate=as.numeric(12)
dec = predict(gbm.caret, newdata=full, type="raw")


sum(abs(nov - full.train1$logerror)) / nrow(full.train1)


submit <- data.frame(full[,c("parcelid")])
submit$"201610" <- round(oct,4)
submit$"201611" <- round(oct,4)
submit$"201612" <- round(oct,4)
submit$"201710" <- round(oct,4)
submit$"201711" <- round(oct,4)
submit$"201712" <- round(oct,4)
submit$p_lr1<- NULL # remove the original prediction from the submit file
nrow(submit)
write.csv(submit, file = "submit_6.csv", row.names = FALSE, na="") # export the file for submission
print('Done!')
